...
export function AppLogo({ className, appName = "Chaos Cash Tracker", ...props }: AppLogoProps) {
  return (
    <div className={`flex items-center gap-2 ${className || ''}`}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 100 100"
        // fill="currentColor" // Use currentColor to inherit from text-primary
        stroke="currentColor"
        strokeWidth="3"
        className="h-10 w-10 text-primary transform rotate-45"
        {...props}
      >
        {/* Chaotic splash/explosion like shape */}
