...
export function IncomeAllocationBar({ income, needs, wants, investments }: IncomeAllocationBarProps) {
  ...
  const segments = [
    { name: "Needs", percentage: needsPercentage, value: needs, color: "bg-blue-500", icon: HandCoins },
    { name: "Wants", percentage: wantsPercentage, value: wants, color: "bg-purple-500", icon: ShoppingBasket },
    { name: "Investments", percentage: investmentsPercentage, value: investments, color: "bg-indigo-500", icon: TrendingUp },
    { name: "Cash Savings", percentage: savingsPercentage, value: savings, color: "bg-green-500", icon: Wallet },
  ...
  return (
     <Card className={cn("shadow-lg", glowClass)}>
        <CardHeader>
          <CardTitle className="text-xl text-primary">Monthly Income Allocation</CardTitle>
          <CardDescription>A visual breakdown of where your income is going this month.</CardDescription>
        </CardHeader>
        <CardContent>
            <TooltipProvider>
                <div className="w-full h-8 flex rounded-full overflow-hidden bg-muted">
