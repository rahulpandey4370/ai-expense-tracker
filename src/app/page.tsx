...
export default function DashboardPage() {
  ...
  if (!isClient) {
    return (
      <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 animate-pulse">
        {/* Removed Date Selector Card Placeholder */}
  ...
  return (
    <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 bg-background">
      {/* Date Selector Card Removed */}

      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <KpiCard title="Total Income" value={`₹${monthlyMetrics.income.toFixed(2)}`} icon={DollarSign} description={`${monthNamesList[selectedMonth]} ${selectedYear}`} />
        <KpiCard title="Total Spending" value={`₹${monthlyMetrics.spending.toFixed(2)}`} icon={TrendingDown} description={`${monthNamesList[selectedMonth]} ${selectedYear}`} valueClassName="text-red-500"/>
        <KpiCard title="Total Savings" value={`₹${monthlyMetrics.savings.toFixed(2)}`} icon={PiggyBank} description={`${monthNamesList[selectedMonth]} ${selectedYear}`} valueClassName={monthlyMetrics.savings >= 0 ? "text-green-500" : "text-red-500"} />
        <KpiCard title="Savings Rate" value={`${monthlyMetrics.savingsRate.toFixed(1)}%`} icon={Percent} description={`${monthNamesList[selectedMonth]} ${selectedYear}`} valueClassName={monthlyMetrics.savingsRate >=0 ? "text-primary" : "text-destructive"}/>
      </div>
       ...
       {monthlyMetrics.spending > monthlyMetrics.income && (
        <Alert variant="destructive" className="shadow-md">
          ...
          <AlertDescription>
            You've spent more than your income in {monthNamesList[selectedMonth]} {selectedYear}. Review your expenses to stay on track.
          </AlertDescription>
        </Alert>
      )}
